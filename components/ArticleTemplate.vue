...
<template>
  ...
  <section class="bg-primary">
    <div class="mx-auto max-w-screen-md px-4 py-8 lg:px-6">
      <div class="mx-auto mb-8 max-w-screen-md lg:mb-12">
        <h1 class="text-color-primary mb-4 text-4xl font-extrabold leading-none tracking-tight md:text-5xl xl:text-6xl">
          {{ title || 'Default Title' }}
        </h1>
        <!-- Only display the description if it exists -->
        <p v-if="description" class="text-color-tertiary mb-5 font-light sm:text-xl">
          {{ description }}
        </p>
        ...
        <div class="mt-4 flex flex-wrap gap-x-10 gap-y-4">
          ...
          <div v-if="readingTime" class="flex items-center">
            <svg class="mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 512 512">
              <path
          ...
          <div v-if="publishingDate" class="flex items-center">
            <svg class="mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 448 512">
              <path
    ...
    <div class="mx-auto max-w-screen-xl px-4 py-8 lg:px-6">
      <div class="mx-auto max-w-screen-md">
        <div v-for="section in body" :key="section.id">
          ...
          <Paragraph v-if="section.type === 'paragraph'" :value="section" class="text-color-tertiary mb-6" />
          ...
          <List v-if="section.type === 'list'" :value="section" class="mb-6" />

          <!-- Render Images component -->
          <Images v-if="section.type === 'image'" :value="section" class="mx-auto my-8 rounded-lg" />
        </div>
      </div>
    </div>
  </section>
